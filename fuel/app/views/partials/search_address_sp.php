<?php Fuel\Core\Config::load('apidefine');?>
	...
	<div id="mainarea">
		...
		<p class="cap pd10 ">※複数の単語で検索する場合は、スペースで区切って入力してください。　例：東京都　大森</p>
		<h4 class="subtit">検索結果</h4>
		<p class="pd10 subtit">住所フリーワード: 「<span class="search-key">神奈川県横浜市戸塚</span>」<span class="rearch-message">で下記の住所候補が見つかりました。住所を選択してください。</span></p>
        ...
        success: function (data) {
			...
			if(data.results){
				...
				$.each( data.results, function( index, value ){
					option += '<li><a href="<?php echo Uri::base().Uri::segment('1'); ?>/maplist?lat='+value.lat+'&lon='+value.lon+'">'+value.name+'</a></li>'
				});
